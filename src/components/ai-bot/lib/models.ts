...
export function getProviderByModelName(modelName: string): string {
  ...
  if (name.startsWith('qwen') || name.includes('阿里') || name.includes('通义')) return 'alibaba'
  if (name.startsWith('glm') || name.includes('智谱') || name.includes('zhipuai')) return 'zhipuai'
  if (name.startsWith('deepseek') || name.includes('deepseek')) return 'deepseek'
  if (name.startsWith('minimax') || name.includes('minimax')) return 'minimax'
  if (name.includes('kimi') || name.includes('月之')) return 'moonshotai'
